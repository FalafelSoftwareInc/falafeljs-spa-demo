define([
	...
	return {
		...
		show: function(message, options) {
			...
			options.close = typeof options.close !== 'undefined'
				&& option.close !== null
				? options.close : true;
		...
		displayValidations: function(errors, container) {
			if (errors) {
				...
				for (var i = 0; i < errors.length; i++) {
					var controlGroup = $('*[name="' + errors[i].Property + '"]', container)
						.closest('.control-group');
						
					if (controlGroup.length > 0) {
						$('.validation-bubble div', controlGroup)
							.html(errors[i].ErrorMessage).parent().show();
					} else {
						...
						content.append('<li>' + errors[i].Property + ": "
							+ errors[i].ErrorMessage + '</li>')
					}
				}
			}
		},
