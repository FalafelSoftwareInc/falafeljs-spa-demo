...
define([
	...
	//USED IN AJAX CALLS FOR PARSE.COM APP INDENTIFICATION
	...
	var toParseCom = function(options) {
		...
		}).success(options.fnSuccess).error(options.fnError).complete(function() {
			//INITIALIZE LOADING PANEL
			...
			if (options.url || options.storage) {
				...
				$.extend(options, {
					...
					fnError: function(err) {
						...
						else Alert.notifyError(options.error 
